...
// Walls (Fixed Duplicate Declaration)
...
const roof = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: '#545151' })
)
...
for (let i = 0; i < 50; i++) {
    ...
    const x = Math.cos(angle) * radius // Get the x position using Math.cos
    const z = Math.sin(angle) * radius // Get the z position using Math.sin
...
gui.add(ambientLight, 'intensity').min(0).max(1).step(0.6)
...
const doorLight = new THREE.PointLight('#ff7d46', 2, 8)
